var Weglot = function () {
  ...
  var logger = createLogger("html-parser-engine");
  var apiEndpoints = {
  ...
  var endpointUrls = Object.keys(apiEndpoints).map(function (key) {
    return apiEndpoints[key];
  });

  function isUrlExternal(url) {
    for (var i = 0; i < endpointUrls.length; i++) {
      if (url.indexOf(endpointUrls[i]) !== -1) {
        return true;
      }
    }
    return false;
  }
  function getCachedResult(key, cache, element, callback) {
    var cachedResult = cache && cache[key];
    if (cachedResult && cachedResult.textContent === element.textContent) {
  ...
  function containsNoTranslateNodes(e) {
    return getCachedResult("__containsNoTranslateNodes", e, function (element) {
      return (
        element.nodeType === 1 &&
  ...
  function isValidMergeNodes(element) {
    return (
      getCachedResult("__validMergeNodes", element, function (el) {
  ...
  function filterValidTextNodes(element, trimWhitespace = true) {
    return getCachedResult("__validTextNodes", element, (node) => {
